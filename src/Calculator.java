...
public class Calculator {
    public static double exponentAlgorithm(double x, int n) {
        if (n == 0) return 1L;
        else return x * exponentAlgorithm(x, n-1);
    }

    public static double exponentAlgorithm2(double x, int n) {
        if (n == 0) return 1;
        else if (n % 2 != 0) { // checks to see if it's an odd number
            return x * exponentAlgorithm2(x*x, (n-1)/2);

        }
        else { // else it's a even number
            return exponentAlgorithm2(x*x, n/2);
        }
    }
}
